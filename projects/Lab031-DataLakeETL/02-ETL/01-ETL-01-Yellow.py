...
# MAGIC %md
# MAGIC Let's load the Yellow cab data.  
# MAGIC 
# MAGIC If we were to look at the transactional data files we would see that the schema "evolves" over time.  You'll see the schema evolution in the code below.  
# MAGIC 
# MAGIC What we want to do is create one canonical schema for all years and load the data into that data frame.  I've written all of the code for you b/c, well, this could take days of hair-pulling and stress.  
# MAGIC 
# MAGIC This will take some time to run, while it is running continue to the next task (loading Green Cab data)
...
# MAGIC %md
# MAGIC Double-check we got our source datadirs correct
...
dbutils.fs.rm(destTranDir,recurse=True)
...
for j in range(2016,2017):
  ...
  for i in range(1,endMonth+1):
    ...
    taxiCanonicalDF.coalesce(6).write.format("delta").mode("append").partitionBy("trip_year","trip_month").save(destTranDir)  
...
# MAGIC USING DELTA
# MAGIC LOCATION '/mnt/lake/raw/nyctaxi/transactions/yellow-taxi/';
