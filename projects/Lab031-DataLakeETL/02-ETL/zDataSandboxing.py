...
# MAGIC %md
# MAGIC # Data Sandboxing
# MAGIC We will run various reports and visualize.  We are simulating what a business analysts or data engineer might do to discover "nuggets of gold" in the data.  
# MAGIC 
# MAGIC Note:  I generally preface notebook names with `z` and then I do not run z notebooks as part of notebook workflows.  Another approach is to have an excluded `documentation` folder, or simply keep these exploratory notebooks outside of the git repo.  
...
# MAGIC %sql
# MAGIC select trip_year,count(*) as trip_count from taxi_db.taxi_trips_mat_view
# MAGIC group by trip_year
